...
def create_features(df):
    ...
    if "trans_date_trans_time" in df.columns:
        ...
        df["dayofweek_sin"] = np.sin(
            2 * np.pi * df["transaction_dayofweek"] / 7
        )
    ...
    if (
        "trans_date_trans_time" in df.columns
    ...
    if all(
        ...
        df["customer_merchant_distance_km"] = (
            haversine_distance(
                df["lat"],
                df["long"],
                df["merch_lat"],
    ...
    if "cc_num" in df.columns:
        ...
        if "trans_date_trans_time" in df.columns:
            ...
            df[
                ...
                .sub(df["previous_transaction_time"])
                .dt.total_seconds()
                / 60
            )
    ...
    if (
        "cc_num" in df.columns
        and "merchant" in df.columns
    ):

        df["card_merchant_count"] = (
            df.groupby(
                ["cc_num", "merchant"]
            ).cumcount()
        )
    ...
    if ("cc_num" in df.columns
        and "amt" in df.columns):

        # Previous Average Amount

        df["previous_avg_amount"] = (df.groupby("cc_num")["amt"]
              .transform(lambda x:x.shift(1) .expanding().mean()))

        # Previous Standard Deviation

        df["previous_std_amount"] = (df.groupby("cc_num")["amt"].transform(
                  lambda x:x.shift(1).expanding().std()))


        # Amount vs Historical Average

        df["amount_vs_historical_avg"] = (df["amt"]/
            df["previous_avg_amount"].replace(0, np.nan))
        # Amount Z-Score

        df["amount_zscore"] = ((df["amt"] - df["previous_avg_amount"])/
            df["previous_std_amount"].replace(0, np.nan))
